...
querystr = "SELECT * FROM c where c.id in ('111','222','333', '123')"
...
for document in query:
      document['new_property'] = "thisisanewdata"
      print(document)
      options = {}
      options['partitionKey'] = document["pk"]

      client.ReplaceDocument(f"""{collLink}/docs/{document['id']}""",document,options=options)