	...
	ReadWriteRatio	=
		CASE
			WHEN IndexUsageStats.user_updates > 0
				THEN FORMAT (CAST ((ISNULL (IndexUsageStats.user_seeks , 0) + ISNULL (IndexUsageStats.user_scans , 0)) AS DECIMAL(19,2)) / CAST (ISNULL (IndexUsageStats.user_updates , 0) AS DECIMAL(19,2)) , N'P')
			ELSE
				FORMAT (0 , N'P')
		END
FROM
