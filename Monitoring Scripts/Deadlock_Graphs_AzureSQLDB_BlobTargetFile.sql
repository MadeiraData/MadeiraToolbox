...
,target_data_XML.value('(/event/data[@name=''database_name'']/value/text())[1]','nvarchar(256)') AS databaseName
...
FROM
(
	SELECT CAST(event_data AS xml)  AS target_data_XML, timestamp_utc
	FROM sys.fn_xe_telemetry_blob_target_read_file('dl', NULL, NULL, NULL)
) AS DLevents
CROSS APPLY
(SELECT victim_process_xml = target_data_XML.query('
...
WHERE target_data_XML.exist('/event/data[@name=''xml_report'']/value/deadlock') = 1
