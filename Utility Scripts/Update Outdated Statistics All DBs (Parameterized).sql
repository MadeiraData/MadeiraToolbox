...
*/
DECLARE
  @MinimumTableRows INT = $(MinimumTableRows) --200000
, @MinimumModCountr INT = $(MinimumModCountr) --100000
, @MinimumDaysOld INT = $(MinimumDaysOld) --35 -- adjust as needed
, @MaxDOP INT = $(MaxDOP) --NULL -- optionally force a specific MAXDOP option. set to 1 to reduce server workload.
, @SampleRatePercent INT = $(SampleRatePercent) --NULL -- set to number between 1 and 100 to force a specific sample rate, where 100 = FULLSCAN
, @ExecuteRemediation BIT = $(ExecuteRemediation) --0 -- set to 1 to execute the UPDATE STATISTICS remediation commands, otherwise print only
, @TimeLimitMinutes INT = $(TimeLimitMinutes) --10 -- time limit in minutes to allow statistics to be updated
