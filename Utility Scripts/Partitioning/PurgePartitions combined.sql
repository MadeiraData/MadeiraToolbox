...
, @MinValueToKeep $(PartitionKeyDataType) = NULL
...
BEGIN
	...
	OR @MaxIntervals > (select COUNT(*) from sys.partition_range_values where function_id = @PartitionFunctionId)
		BREAK;
...
ALTER PARTITION FUNCTION ' + QUOTENAME(@PartitionFunctionName) + '() MERGE RANGE (@MinPartitionRangeValue);'
	...
	PRINT CONCAT(N'Merging @MinPartitionRangeValue: ', @MinPartitionRangeValue)
	RAISERROR(@CMD,0,1) WITH NOWAIT;
